...
@property (nonatomic) NSMutableDictionary *standardBtnClickDict;//记录被按下的btn 的tag
@property (nonatomic) UITableView *mainTableView;
@property (nonatomic) NSMutableArray *standardBtnArr;//每一个元素是一个属性的Btn数组
...
//    cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, StandardViewHeight-44, StandardViewWidth/2, 44)];
    ...
    cancelBtn.frame = CGRectMake(ScreenWidth - 40, 10, 30, 30);
    cancelBtn.tag = 1000 + 1;
//    cancelBtn.backgroundColor = MainYellowColor;
...
//
    sureBtn = [[UIButton alloc] initWithFrame:CGRectMake(StandardViewWidth/2, StandardViewHeight-46, StandardViewWidth/2, 46)];
    sureBtn.tag = 1000 + 2;
...
-(void) initTableView
{
    ...
    self.goodNum.textColor = [UIColor colorWithHex:0x999999];
    self.goodNum.font = [UIFont systemFontOfSize:16];
...
{
    if (self.standardArr.count != [self.standardBtnClickDict allKeys].count) {
        NSLog(@"----%@---",[self.standardBtnClickDict allKeys]);
//       写个Block 提示选中 规格
        self.ShowBlock(@"请先选择商品规格");
        return;
    }
      ...
      */
    if (sender.tag == 0) {
        if (self.goodAttriNumber < self.buyNum +1 ) {
            self.ShowBlock([NSString stringWithFormat:@"亲，这款商品只有%ld件了",(long)self.goodAttriNumber]);
            return;
        }
...
//            [alertView show];
    ...
    // 刷新价格
   self.priceLab.text = [NSString stringWithFormat:@"￥%.2f",[self.price floatValue]* self.buyNum];
    if ([self.delegate respondsToSelector:@selector(StandardsView:GoodsNumber:AndTotalPrice:)]) {
        [self.delegate StandardsView:self GoodsNumber:self.buyNum AndTotalPrice:self.priceLab.text];
    }
...
//    UITableViewCell* myCell = (UITableViewCell *)[sender superview]; //表示Button添加在了Cell中。
    //如果将Button添加在myCell.contentView 中，
     UITableViewCell* myCell  = (UITableViewCell *)[[sender superview] superview];
    
    NSIndexPath * cellPath = [self.mainTableView indexPathForCell:myCell];
    NSInteger cellrow = cellPath.row; //这个就是cell的indexPath.row;    NSArray *tempArr = self.standardBtnArr[sender.tag];
    NSArray *tempArr = self.standardBtnArr[cellrow];
    
    for (UIButton *tempBtn in tempArr) {
     
        tempBtn.backgroundColor = [UIColor colorWithHex:0xececec];
        [tempBtn setTitleColor:[UIColor colorWithHex:0x666666] forState:UIControlStateNormal];
//        tempBtn.selected = NO;
    }
...
//    NSString *tagStr = [NSString stringWithFormat:@"%ld",(unsigned long)(sender.tag & 0xffff0000)>>16];
    ...
    [self.standardBtnClickDict setObject:tagStr forKey:[NSString stringWithFormat:@"%ld",cellrow]];
    
    if([self.delegate respondsToSelector:@selector(Standards:SelectBtnClick:andSelectID:andStandName:andIndex:)])
    {
        NSString * selectName ;
        for (standardClassInfo *standarinfo in _standardArr[cellrow].standardClassInfo) {
            if ([standarinfo.standardClassId isEqualToString:tagStr]) {
                selectName = standarinfo.standardClassName;
             }
        }
        
        
        [self.delegate Standards:self SelectBtnClick:sender andSelectID:selectName andStandName:self.standardArr[cellrow].standardName andIndex:sender.tag/100];
    }
...
//        return cell;
    ...
    @try {
        ...
        if(indexPath.row < self.standardArr.count)
        {
           ...
           UILabel* titleLab = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, SCREEN_WIDTH, 30)];
            ...
            titleLab.textColor = [UIColor colorWithHex:0x333333];
    ...
    //        NSString *strID = [NSString stringWithFormat:@"%@",dicGoodsDetail[@"spec_name"][indexPath.row][@"id"]];
            ...
            for (int i = 0; i < specArr.count; i++) {
                ...
                btn.backgroundColor = [UIColor colorWithHex:0xececec];
...
//                btn.tag = indexPath.row*100 + i/*低16位 cell行数*/ | ([specArr[i].standardClassId intValue] << 16) /*高16位  分类id*/;
                btn.tag =  [specArr[i].standardClassId intValue] ;
        ...
        else
        {
            ...
            plusBtn.tag = 0;
            [plusBtn setImage:[UIImage imageNamed:@"加"] forState:UIControlStateNormal];
            [cell addSubview:plusBtn];
            
            self.numberTextFied.frame = CGRectMake(plusBtn.frame.origin.x - 40, plusBtn.center.y -10, 40, 20);
            self.numberTextFied.textAlignment = NSTextAlignmentCenter;
            self.numberTextFied.backgroundColor = [UIColor colorWithHex:0xececec];
            self.numberTextFied.textColor =[UIColor colorWithHex:0x999999];
            self.numberTextFied.userInteractionEnabled = NO;
