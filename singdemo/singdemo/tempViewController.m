...
-(void)makemenuScrollow{
    ...
    self.DataArray = @[@"全部",@"待支付",@"已支付",@"已使用",@"未使用",@"预期",@"赠送"];
    
    // 创建滚动菜单栏
    self.menuView = [[CQScrollMenuView alloc]initWithFrame:CGRectMake(0,NavStatusBarHeight, self.view.width, 40)];
    [self.view addSubview:self.menuView];
    self.menuView.menuButtonClickedDelegate = self;
    self.menuView.lineColor = [UIColor colorWithHex:0x06a27b];
